...
// validate email Or Number format
$.validator.addMethod("emailOrNumber", function(value, element) {
	var emailOrNumber = value.split(","),
		isValidate = true;
	var validateformat  = function(str , f){ f  = f ? (/^[a-zA-Z0-9._-]+@[a-zA-Z0-9-]+\.{1,1}[a-zA-Z]{2,4}$/i) :(/^[0-9]{1,}$/i); return f.test(str);  }
	
	if(emailOrNumber.length){
		$.each(emailOrNumber , function(i , currentEntry){
				// test on entry type | email || tel
				currentEntry = $.trim(currentEntry);
				if(currentEntry.indexOf('@')>-1){
					// case : Email format
					if(!validateformat(currentEntry , true)){
						isValidate = false;
						return false;
					}	
				}else{
					// case : Tel
					if(!validateformat(currentEntry)){
						isValidate = false;
						return false;
					}	
				}
...
// validate email Or Number format
$.validator.addMethod("emailOrNumberWithoutComa", function(value, element) {
	var emailOrNumber = value.split(","),
		isValidate = true;
	var validateformat  = function(str , f){ f  = f ? (/^[a-zA-Z0-9._-]+@[a-zA-Z0-9-]+\.{1,1}[a-zA-Z]{2,4}$/i) :(/^[0-9]{1,}$/i); return f.test(str);  }
	console.log(value);
	if(value.indexOf(',')>-1)isValidate = false; 
